...
def adjust_learning_rate(optimizer, warm_up, epoch, epochs, base_lr, step, iteration_per_epoch):  
    ...
    if T < warm_up:  
        ...
        min_lr = 0  
        T_epoch = epoch+1 - warm_up / iteration_per_epoch  
        total_epochs = epochs - warm_up / iteration_per_epoch  
        lr = 0.5 * (1 + math.cos(1.0 * T_epoch / total_epochs * math.pi)) * (base_lr - min_lr) + min_lr  
...
def train_one_epoch(model, optimizer, data_loader,loss_function, device, epoch, epochs, warm_up, base_lr):
    ...
    return mean_loss.item(), sum_num.item(), epoch_lr
